...
`exit(code)`は`main`の`return`以外でプログラムを即座に終了させる組み込み関数です。呼び出した場所がどこであっても構わず、その場でプロセス全体が終わります——ただし`return`と違い、パニックの捕捉手段`recover(...)`(`weave_spec.md` §17)による捕捉は一切効きません(`os.Exit`自身の挙動そのままで、Goの`defer`機構を経由しないため)。
