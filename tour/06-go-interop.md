...
## Go関数の宣言と呼び出し(`weave_spec.md` §14.1〜§14.2)
...
**Go関数・Goメソッドの呼び出しは、戻り値の個数によらず常に3・4章で見た`list`(連番の数値キーを持つオブジェクト)を返します。** Weaveには複数戻り値という概念が無いため、`at(list, index)`で個々の値を読み出します。`toUpper(...)`は本来1つの文字列しか返しませんが、それでも`at(..., 0)`で取り出す必要があります。すべての呼び出しはreflectionを経由します(14.6節)。
...
## プロトタイプの結びつけ(`weave_spec.md` §14.2)
...
## `govar`——Goのパッケージレベル変数を読む(`weave_spec.md` §14.4)
...
- 呼び出したGo関数・メソッドが(エラー値を返すのではなく)Go自身の`panic`を起こした場合、デフォルトではWeave側で捕まえる方法はありません——ただし8章の`recover(...)`を使えば、この種のパニックも他の実行時パニックと同様に捕捉できます
