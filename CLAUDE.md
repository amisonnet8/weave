...
### `&&`/`||`はまだ短絡評価しない(Step 3の既知のギャップ)

`weavert.And`/`Or`は普通のGo関数呼び出しとして実装したため、**呼び出し前にGoが両方の引数を評価する**(短絡評価にならない)。Step3の時点ではWeaveに副作用を持つ式(オブジェクトのプロパティアクセス失敗等)がまだ無いため実害は無いが、意味論としては仕様の暗黙の期待(多くの言語の`&&`/`||`の通例)からずれている。**正しい短絡評価にはIF/GOTO/LABELによる分岐が必要で、これはStep4(制御構文)で`if`と一緒に導入する予定。** Step4で`weavert.And`/`Or`をcodegenレベルの分岐に置き換えること。
