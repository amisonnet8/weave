...
`examples/`はまだこの機能専用のサンプルを持たない(今後の課題として残す)。単体テスト(parser/sema/codegen)を新規・置き換えで追加し、全て green。amivm実地検証: `fn()`/`f()`の正常系、`obj.method()`が影響を受けないこと(`self`のみの1回の`weavert.Call`で、`nil`の追加適用が無いこと)、`_`の読み取り・再代入それぞれの実行時ではなくコンパイル時エラー、`_`が独立した兄弟・ネストスコープで衝突しないこと、`list()`/`goReturns()`/`goParams()`が無変更で動作すること、を全て確認した。
