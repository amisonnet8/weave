...
### `list[index]`の添字読み書き構文を追加(19節の別項目、ユーザーからの追加依頼)

複数行コメントの完了後、ユーザーから19節が挙げる別の制約(角括弧構文が一切存在しない)のうち、`list[i]`(添字アクセス)への対応を「構文糖衣でいけないか」という提案付きで依頼された。

**読み取りだけなら`at(list, i)`への単純なパーサー時点の構文糖衣で済むが、書き込み(`list[i] = v`)への対応も必要かをユーザーへ確認した。** `at(...)`には元々書き込み側が存在せず、書き込みまで対応する場合は影響範囲が変わる(新しいAST/ランタイム関数が要る)ため、読み取り専用か読み書き両対応かを1問だけ聞いた——回答は「読み書き両対応」。

**`[`/`]`はWeaveが字句解析器に追加する初めての新しい記号トークンだった。** これまでの全ての構文拡張(`gotype`/`gofunc`/`govar`/`package(...)`/0引数関数糖衣/複数行コメント)は、既存のトークン・構文形状(`CallExpr`のパターンマッチ、または字句解析層だけで完結する変更)の範囲に収まっていたが、今回は本当の意味で新しいプリミティブ構文(後置演算子`[expr]`)の追加になる。

**読み取り側を`at(...)`への即時パーサー展開にしなかった、実装上のキーポイント。** 当初は`list[i]`を見た瞬間に`CallExpr{Callee: Ident{"at"}, Args:[list, i]}`へパース時点で展開しようとしたが、これだと`parseSimpleStmt`が「代入のLHSが`[]`付きの式だったかどうか」を後から判定できなくなる(`at(...)`という普通の呼び出しと区別が付かなくなるため)——これは0引数関数糖衣構文(直前の節)で確立した「情報を失う変換は、その情報がまだ必要な地点より手前で行ってはいけない」という教訓の再演だった。対策として`ast.IndexExpr`(読み取り用)・`ast.IndexAssignStmt`(書き込み用)という2つの新規ASTノードを、既存の`ast.PropExpr`/`ast.PropAssignStmt`(`obj.name`/`obj.name = value`)と全く対称な形で新設し、`parseSimpleStmt`が`=`の直前の式の型(`Ident`/`PropExpr`/`IndexExpr`)を見て3種類の代入文を振り分ける、という既存の分岐にそのまま1ケース追加するだけで済んだ。

**codegenは`weavert.ObjAt`(既存)・`weavert.ObjSetAt`(新設)への単純な1行`CALL`に落ちるだけで、`list[i]`は`at(list, i)`と完全に同一のIRを生成する。** 新設した`ObjSetAt`は、`ObjAt`の「範囲外はその場でパニック」という既存の意味論(11節)を書き込み側にも対称に適用し(存在しない位置への書き込みは失敗させる)、`list(...)`/`at(...)`が元々持つ「要素の追加・削除・挿入・並べ替えといった配列操作が無い」という制約(11節)を、角括弧の書き込みでも崩さないようにした——書き込みを「常に成功する`ObjSet`相当」にしてしまうと、この制約を暗黙に破ることになったはずで、実装前の設計段階でこの対称性を意図して選んだ。

**発見: AST・codegen本体だけを更新して満足せず、AST全体を横断的に歩く別の場所(`internal/modloader/rewrite.go`)を見落とし、実地検証で初めて発覚した。** モジュール機構(17節)の`package(...)`解決パスは、sema/codegenとは独立に自前でAST全体を辿ってリネーム・修飾子解決を行っており、この`rewriteExpr`/`rewriteStmt`という2つの網羅的な`switch`にも新しいノード種別(`*ast.IndexExpr`・`*ast.IndexAssignStmt`)を追加する必要があった——追加を忘れた状態では`modloader: unsupported expression *ast.IndexExpr`という実行時エラーになり、単一ファイルの実地検証だけでは発覚しなかった(モジュール機能を使うプログラムで初めて踏む)。**新しいASTノードを追加する際は、`grep`で「既存の類似ノード(`PropExpr`/`PropAssignStmt`)を`case`に持つ全てのファイル」を洗い出し、codegen/sema以外にもASTを独立に歩く場所(modloaderのような)が無いか確認すること**、という教訓——このプロジェクトでASTノードの種類そのものを増やしたのは今回が初めてだったため、今まで気づかれていなかった横断的な網羅性の要求だった。

`examples/indexing.weave`を新設し、読み取り・書き込み・`[i][j]`のような連続添字・`.prop`との自由な混在(`obj.items[i]`、`list[i].prop`)を一通り実演し、`amivm`→`go build`→実行まで実地検証した。範囲外書き込み・非数値インデックス書き込みのエラー、モジュール機構(`package(...)`)をまたいだ`list[i]`の使用、をそれぞれ個別に実地検証した。単体テスト(lexer/parser/sema/codegen/weavert)を新規追加、全て green。既存の全18サンプル(新設の`indexing.weave`含めて19)+`examples/modules`+`examples/multifile`の回帰も無変更で動作継続することを確認した。

### `for k, v in list(...)`を数値順にする——listのキーを10桁ゼロ埋め文字列で保持(`list[index]`の直後に追加)

`list[index]`(直前の節)の完了後、ユーザーから7節・19.6節が挙げる別の既知の制約(`for k, v in obj`はキーを文字列としての辞書順で列挙するため、要素数が10個以上の`list(...)`だと`"10"`が`"2"`より先に来る)への対応を、「listのkeyは10桁の0パディングした値で持つ。`at`や`list[index]`でアクセスするときは0なしでもできるようにする」という具体的な実装方針の提案付きで依頼された。

**提案通りの方針で実装可能と判断し、そのまま実装した。** キーを固定桁数のゼロ埋め文字列(`"0000000000"`〜)で保持すれば、文字列としての辞書順ソートが数値としての大小順と一致するのは自明であり、ユーザー自身の提案が既にほぼ完成した設計だった——確認を要する分岐点は無く、実装のみで対応した。

**唯一自分で判断を加えたのは適用範囲——ユーザーが明示したのは`at`/`list[index]`だが、`has`/`remove`/`.`(内部的には`ObjGet`/`ObjSet`)にも同じ正規化を及ぼした。** これらは全て同じ1つの choke point、`weavert.keyOf`(文字列キーを受け取る箇所)を経由するため、「渡された文字列が全て数字なら10桁ゼロ埋めへ正規化する」という変換を`keyOf`自身に仕込むだけで、`ObjGet`/`ObjSet`/`ObjHas`/`ObjRemove`/`at`/`list[index]`の全てが自動的に一貫した挙動になった。`at(list, 5)`と`has(list, "5")`が違う要素を指してしまう(片方だけ正規化されている)という不整合を生まないための判断で、ユーザーの提案の範囲を実装の一貫性のために自然に広げた形——新しい設計判断というよりは、提案されたルールを筋の通った形で最後まで適用しただけだった。

**`for k, v in obj`に見せる`k`は、ゼロ埋め前の元の文字列へ戻す(`stripListKeyPadding`)。** ソートは内部のゼロ埋めキーで行うが、Weaveプログラムから見える`k`は今まで通り`"0"`,`"1"`,...`"11"`のまま——ゼロ埋めはあくまで内部のソート順を正すための実装詳細であり、外部から観測可能にする理由が無いため、`ObjKeys`がソート**後**に全キーへ`stripListKeyPadding`を適用してから返すようにした。
...
### 型ヒント(`shape`/`checkShape`・`goReturns(...)`/`goParams(...)`)を削除——`gofunc`/`gomethod`は常にreflect経由、プロトタイプの結びつけは`protoOrNil`方式へ回帰

ユーザーから「型ヒントが中途半端になってしまった」という評価を受け、4.3節の`shape`/`checkShape`(Weave自身のオブジェクト向け)と15.4節の`goReturns(...)`/`goParams(...)`(Go資産境界向け)の2機能を丸ごと削除した。どちらも「型を書けば実行時にASSERTで厳密に検証する、TypeScriptに対するJavaScriptと同じ位置づけの任意機能」として設計・実装したものだったが(上記「Go資産境界への型ヒント」「Weave自身のオブジェクトへの型ヒント」の各節)、機能自体の複雑さ(all-or-nothing制約、スライス型ヒントの要素型限定、数値型の双方向変換、`METHOD`命令の追加まで要した`gomethod`の型付き経路等)に見合う価値を持たないと判断された。

**実装着手前に、削除が起こす副作用を1つ発見しユーザーへ確認した。** `goReturns(...)`は型ヒント機能であると同時に、Go資産の戻り値list(15.2節の「常にlist」ルール)の**どの位置がどの`gotype(...)`プロトタイプと結びつくか**を宣言する唯一の手段でもあった(旧`gofunc(name, protoOrNil)`という2引数の未型付きプロトタイプ結びつけが、常にlist化の再設計の際に`goReturns(...)`へ吸収されていたため)。これに気づかず`goReturns`/`goParams`を単純に削除すると、`gotype`/`gofunc`自体は残っても`r = at(newReader(...), 0); r.len()`のような連鎖呼び出しが一切できなくなり、`examples/gomethods.weave`・`examples/goassets.weave`・`examples/integration.weave`が軒並み壊れる——ユーザーに選択肢を提示し、「`protoOrNil`引数を復活」(推奨案)を選んでもらった。

**削除後の設計**: `gotype`/`gofunc`/`gomethod`はGo資産連携の基本(15.1〜15.3節)としてそのまま残る。`gofunc(名前)`/`gomethod(名前)`は常に`weavert.CallGoFuncList`/`CallGoMethodList`(reflect経由)で呼ばれ、戻り値の個数によらず常にlistを返す(15.2節の「常にlist」ルールは削除の対象外——変わらず維持)。任意の**第2引数**(`gotype(...)`で宣言済みの名前)が「プロトタイプの結びつけ」——返ってきたlistの**位置0**をそのGo構造体/ポインタ値だとコンパイル時に記録し、`at(result, 0)`で取り出した値への`.Method()`呼び出しがメソッド**名**だけ静的に解決される(Go呼び出し自体は引き続きreflect経由——16節で明記した通り、「静的解決」の実質的な意味は常にここまでだった)。旧`GoReturnSpec`の`Type`フィールド(Goの具体的な型トークン)は不要になった——ASSERT/METHOD/FNTYPEを一切使わないため、`Proto`(結びつけ先の`gotype`名、位置0のみ)という1つの文字列フィールドだけで足りる。

**実装は大幅な削除が中心だった。** `internal/codegen/goasset.go`から`genNativeGoMethodCall`・`genNativeReturnValue`・`genAssertOrTypeError`・`genAssertByteSliceParam`・`genAssertGeneralSliceParam`・`genAssertNumericParam`・`ensureVariable`・`goTypeToken`・`sliceElemGoFuncSuffix`・`numericGoTypeNames`・`nativeReturnConversion`・`genBoxList`・`callTargetsPrefix`を削除し、`genGoFuncCall`/`genGoMethodCall`は常に`CallGoFuncList`/`CallGoMethodList`への単純な1本のCALLへ戻した。`internal/codegen/shape.go`・`internal/sema/shape.go`は丸ごと削除。`weavert`の`TypeError`(ASSERT失敗時のエラー整形専用)・`IsWeaveFunc`(shape の`function`種別チェック専用)・`slices.go`(スライス型ヒントの要素ごと変換関数一式)も、どちらの機能にも使われなくなったため削除した——`weavert.CallGoFuncList`/`CallGoMethodList`/`RaiseIfError`/`NormalizeGoValue`(§15.2の「常にlist」・§11の`raiseIfError`用)は無変更で残る。`codegenCtx`の`goBytesTypeEmitted`/`goSliceTypeNames`/`topLevelDecls`(型ヒント専用の`SLTYPE`宣言インフラ)、`funcGen.newUndeclaredTemp`(`METHOD`命令専用)もまとめて削除し、`Generate()`のトップレベル`SLTYPE`出力ロジックごと消えた——AMIVM側の`METHOD`命令自体はamivm本体に残っているが、Weaveはもう使わない。

sema側の静的追跡(`goStaticVars`/`goListShapes`)は、複数戻り値位置のプロトタイプ配列(`[]string`)から**単一の文字列**(`goListProto[名前] = プロトタイプ名`、常に位置0のみ)へ単純化した——型ヒントが無くなった以上、複数位置がそれぞれ別のプロトタイプを持つケースを表現する理由が無くなったため。`gotype`/`gofunc`宣言のパース(`checkGoTypeDecl`/`checkGoFuncDecl`/`checkGoMethodArgs`)も「名前のみ(1引数)」または「名前+プロトタイプ名(2引数)」の2形だけになり、旧`goReturnsArg`/`goParamsArg`/`validateSliceHint`/`goTypeArg`/`goTypeArgs`/`sliceElemSupported`は全て削除した。
...
### モジュールの内部一意化プレフィックスをディレクトリ名から`package(...)`の代入先名へ変更——同名ディレクトリ衝突バグを修正(「検討中の今後の対応」§1として検討済みだった第2版を実装)

「検討中の今後の対応」に記録していた第2版の設計(下記参照)をそのまま実装した。**設計自体は前回の協議で既に固まっていたため、実装フェーズでの主な作業は「第2版のまま素直に実装した場合に生まれる新しい論点(同じ束縛名を別々のパッケージへ割り当てるケース)を、既存のテストとどう整合させるか」の精査だった。**

**核心の変更**: `internal/modloader/modloader.go`の`loadPackage`に`prefix string`引数を追加し、`extractPackageDecls`が`ld.loadPackage(targetLoc, "", false, a.Name)`と自分自身のAssignStmtの`Name`(`package(...)`の代入先の変数名)を渡すようにした。`collectRenames(file.TopLevel, prefix)`はこの`prefix`を使う——`loc.name`(ディレクトリのベースネーム)はもう一意化プレフィックスの生成に使われず、エラーメッセージ用の表示名(`loadedPackage.Name`)としてのみ残る。これにより、`isValidPackageName`(ディレクトリ名が識別子として妥当な形かを検査していたチェック)は丸ごと不要になり削除した——プレフィックスの供給源が`package(...)`の代入先名(パーサーが既にレキサーレベルで識別子妥当性を保証済み)に変わった以上、この検査自体が意味を失ったため。

**実装前の分析で見つけた、設計を1段複雑にする必要があった論点: 「別々の場所で同じ束縛名を別々のパッケージへ割り当てた」場合のエラーは、既存の`TestLoad_ReassigningQualifierToADifferentPackageUsesTheLastOne`(1つのファイル内で`shared`を`package("./a")`→`package("./b")`と再代入する既存テスト、「後の方が勝つ」という仕様として確立済みだった)と機構的に全く同じケースだと気づいた。** 「別々のファイルが独立に同じ名前を選んでしまう」というCLAUDE.mdの例示は表面的には「クロスファイル」のケースを指していたが、実装レベルでは両方とも「同じ`prefix`文字列で、異なる絶対パスに対して`loadPackage`が2回呼ばれる」という同一の状況に帰着する——ここで区別を付けようとすると(例:「同一ファイル内の再代入だけは特別に許す」)、まさにこの機能が解決しようとしている「一意化プレフィックスの衝突」というバグをこのケースだけ再び許してしまうことになる(`shared_X`が`a`パッケージの値と`b`パッケージの値の両方から出力され、マージ後のTopLevelで衝突する)。**そのため「同一ファイル内の再代入」と「別ファイルでの独立した重複」を区別せず、両方とも同じ1つのグローバルな登録簿(`loader.prefixOwners map[string]string`、prefix→最初にそのprefixを使った絶対パス)でチェックする設計にした。** 結果として`TestLoad_ReassigningQualifierToADifferentPackageUsesTheLastOne`は「後の方が勝つ」という前提が成立しなくなったため、`TestLoad_ReassigningBindingNameToADifferentPackageIsAnError`(同じ束縛名の再代入がエラーになることを検証するテスト)へ置き換えた——この置き換え自体もCLAUDE.mdの第2版検討時点の記述(「ユーザーと協議の上、これは分かりやすいコンパイルエラーにする方針で確定した」)と整合する、想定内の帰結だった。

`prefixOwners`は同一絶対パスの再ロードには反応しない——`loadPackage`冒頭の`ld.loaded[loc.key]`によるキャッシュチェックが、プレフィックスの確認より先に早期returnするため(ダイアモンド輸入で同じパッケージが複数箇所から異なる/同じ束縛名で取り込まれても、最初にロードした時の束縛名がプレフィックスとして生き残り続ける、という第2版の設計方針通り)。

**この変更にはもう1つの直接的な帰結があった: ディレクトリ名がWeaveの識別子として妥当な形である必要が完全に無くなった**(`isValidPackageName`削除の結果)。旧`TestLoad_InvalidPackageNameIsAnError`(ハイフンを含むディレクトリ名`my-utils`の取り込みをエラーとして拒否するテスト)は、このチェックがそもそも「ディレクトリ名がプレフィックスとして使われる」という前提の上にしか成り立たなかったため、`TestLoad_DirectoryNameNeedNotBeIdentifierShaped`(同じ`my-utils`ディレクトリが、`u = package("./my-utils")`という取り込みで問題なくコンパイルできることを検証)へ置き換えた——制約の撤廃自体は今回の設計変更の直接の副作用であり、追加のユーザー判断を要するものではないと判断した。

**実地検証**: この機能が修正する不具合そのもの(`foo/utils`・`bar/utils`という同名ディレクトリの2パッケージを、`utilsA`/`utilsB`という別々の束縛名で取り込む)を`weave run`で再現し、旧実装なら`B:1`/`B:2`(片方がもう片方を上書き)になっていたところが、新実装では`A:1`/`B:2`(正しく別々に扱われる)になることを確認した。新しいエラーケース(同じ束縛名`utils`を2つの異なるパッケージへ割り当てる)も`weave build`で実地検証し、`binding name "utils" is already used to import ".../utilsA" — package(...) can't reuse it for ".../utilsB" too`という分かりやすいメッセージで拒否されることを確認した。単体テスト(`internal/modloader`)を新規追加・置き換え(`TestLoad_SameDirectoryNameDifferentPackagesNoLongerCollide`でバグ修正そのものを、`TestLoad_SameBindingNameForDifferentPackagesAcrossFilesIsAnError`でクロスファイルの衝突エラーを、`TestLoad_ReassigningBindingNameToADifferentPackageIsAnError`で同一ファイル内再代入の衝突エラーを、`TestLoad_DirectoryNameNeedNotBeIdentifierShaped`で識別子制約の撤廃を、それぞれ個別に検証)、全て green。既存の全19サンプル+`examples/modules`+`examples/multifile`の回帰(`make test-examples`)、および`examples/modules`の実行結果(`10`/`100`/終了コード`3`)も無変更で動作継続することを確認した。weave_spec.md §17.4・§19.8を新しい仕様(プレフィックスの供給源・新しいエラーケース)に合わせて全面的に書き換えた。

### Goのパニックを捕捉する機能: `recover(handler)`(「検討中の今後の対応」§1として検討済みだった論点を実装)

「検討中の今後の対応」に記録していた3つの未確定論点(対象範囲・構文・アクターとの関係)を、実装着手前にユーザーへ改めて確認してから実装した。**「判断に困る場合は聞くように」という指示を受け、AskUserQuestionで2問(捕捉対象の範囲・アクター内利用の可否)を確認した**——構文(`defer`スタイルのビルトイン)はCLAUDE.mdに既に「ユーザーの想定」として記録済みだったため、質問対象から外し、そのまま踏襲した。

**回答: 1. 捕捉対象範囲は「Weave自身の実行時エラーも含める」(広い方)。2. アクターのメッセージハンドラ内での利用は「使用不可のままにする」(推奨案通り、§6.5の原則を維持)。**

**スコープの広さは実装方法に影響しない、という点を実装前の分析で確認できた。** Weaveの全てのパニック(`ASSERT`失敗・`weavert`の演算子型エラー・Go資産由来のreflectパニック)は、実装レベルでは全て同じGoの`panic(...)`でしかない——Goの`recover()`自体、パニックの発生源を一切区別しない。そのため「対象範囲を広くする」という決定は、`weavert.RecoverPanic`の実装に一切の分岐を要求しなかった(スコープの広さは、この機能がAMIVM-IR側で単一の`DEFER`命令に落ちるという実装のシンプルさに影響しない)——影響するのはもっぱらweave_spec.md側の記述(§19.2の「例外処理は無い」を「狭い回復手段はある」へ書き換える必要があった、など)だった。

**アクター内での禁止は、実装してみて初めて「完全な禁止は原理的に不可能」と分かった論点だった。** Weaveの統一原理(0節: メッセージハンドラの定義はオブジェクトのメソッド定義と全く同じ構文)により、**ある関数リテラルが「将来メッセージハンドラとして使われるかどうか」はコンパイル時に判定できない**(同じ`fn(self, x) {...}`が、ただのメソッドとしても、`spawn`されたオブジェクトのハンドラとしても、両方に使われうる)。実装前の分析で、この曖昧さを解消する2つのアプローチを比較した:

1. **ランタイム側で防ぐ**(アクターの受信ループが走っているゴルーチンの内部では`recover()`を無効化する): Goには標準のgoroutine-local storageが無く、`(*Actor).run`が呼び出す`Call(handler, state)`から`weavert.RecoverPanic`まで、余分な文脈を一切引き回さずに「今アクター内か」を判定する簡潔な手段が無い(unsafeなgoroutine ID取得や、全呼び出し経路への文脈引き回しが必要になり、実装コストが見合わない)
2. **静的(構文的)にオブジェクトリテラルのフィールド値である関数リテラルを検出し、その内部の`recover(...)`をコンパイルエラーにする**(採用): このプロジェクトが繰り返し採用してきた「構文的な検査で、間接参照までは追わない」という設計判断(9節の自己注入糖衣構文の判定、17節の`修飾子`シャドーイング検査等)の延長線上にあり、実装コストも低い

**2を選んだ根拠: 完全な防御(あらゆる間接呼び出し経路を塞ぐ)は、このプロジェクトが他のどの機能でも達成していない水準の要求であり、ここだけ例外的に高いコストを払う理由が無いと判断した。** 静的検査は、`checker.inHandlerLiteral`という1つのフラグを`checkFuncLit`(sema.go)へ通すだけで実装できた——このフラグは、オブジェクトリテラルのフィールド値として直接渡された`*ast.FuncLit`をチェックする際に`true`にセットされ、それ以降**リセットされることなく**、ネストした全てのクロージャーのチェックへ伝播する(「一度trueになったら、その関数リテラルの構文的な子孫全てに効く」という単純な伝播規則)。これにより、ハンドラの本体に直接書いた`recover(...)`だけでなく、ハンドラ内部でさらに定義した入れ子のクロージャー内の`recover(...)`も一律に拒否される——実装前の分析で「入れ子のクロージャーがハンドラの動的な実行範囲の一部として動く以上、そこでの`recover()`も同じように障害分離を破りうる」と気づけたため、最初からこの伝播込みで設計できた。**既知の限定範囲(受け入れ済み)**: 一度別の変数へ代入したクロージャーをオブジェクトリテラルのフィールドへ間接的に渡す、という迂回は構文的な検査をすり抜ける——他の類似機能と同じトレードオフ。

**実装は`weave_implementation_notes.md`§19に記録した通り、Weaveのほぼ全ての組み込み関数が従う「`?weavert.Xxx`への`CALL`一本化」パターンから外れる、唯一の例外になった。** Goの`recover()`は、それを直接呼び出した関数が`defer`で登録されている場合にしか効果を持たないため、`weavert.RecoverPanic`(内部で`recover()`を呼ぶ)自体が本物の`DEFER`で登録されていなければならない——`weavert.Call`のような間接呼び出しヘルパーを経由すると、`recover()`が常に`nil`を返す壁にぶつかる。対策として`recover(handler)`だけは、他の組み込み関数のような`CALL`ではなく、`DEFER ?weavert.RecoverPanic <handler>`という単一のネイティブ命令へ直接変換した(`internal/codegen/codegen.go`の`genRecoverCall`)。

**「回復後、関数はどう終わるか」もGo自身の`defer`+`recover()`セマンティクスに完全に委ねることで、Weave側の特別な実装が不要になった。** Goの無名(named returnでない)戻り値は、deferで回復した場合そのゼロ値になる——実際に生成されたGoコードを確認したところ、`weave_main`自身の戻り値型は`^int`と宣言時点で確定しており(`Generate()`の`FUNC !weave_main ^any : ^int`)、`main`の`return`は毎回`weavert.ExitCode`変換を経てから`^int`としてRETされるため、`main`が`recover(...)`で回復した場合はGoの`int`ゼロ値である`0`がそのまま終了コードになる——`weavert.ExitCode(nil)`が`panic`するような二次災害を心配する必要は無かった(直接probeで`weave emit-go`の出力を確認し、`weave_main_amivm_function`の戻り値型が素の`int`であることを確認してから設計を確定させた)。通常のクロージャーは元々`^any`型で、既に「本体が明示的な`return`を持たなくても`nil`を返す」(Step7の確定判断)という前例があるため、回復時の`nil`はこの延長として自然に収まる。

**`handler`自身の戻り値は捨てられる(Goが無名戻り値をdeferから変更する手段を持たないため)——この制約から「値を持ち帰るには、専用のヘルパークロージャーの内側で外側の変数へ直接書き込む」という利用パターンが導かれ、weave_spec.md §20.3にそのパターンを明記した。** ドキュメント執筆中、当初書いた例(`result = guarded()`のようにガード用クロージャーの戻り値を直接受け取る形)が、実際に`amivm`→`go build`→実行で確認したところ**誤りだと判明した**——`guarded()`が`return riskyOperation()`という形でパニックしなかった場合の結果を返す設計だと、パニックして回復した場合は`guarded()`自身の戻り値が(Goのゼロ値セマンティクスにより)常に`nil`になり、その`nil`が`result = guarded()`という代入によって、直前に`handler`が書き込んだはずの`result`を踏み潰してしまう。修正として、`guarded`自身の戻り値を一切使わず、成功・失敗どちらの経路でも`result`という外側の変数へ直接書き込む形(`guarded`は裸の文として呼び出すだけ)に例を書き換えた——**ドキュメントに書いたコード例も必ず実行して確認する**、というこのプロジェクトの一貫した規律が、ここでも実際にバグを1つ捕まえた。

`examples/recover.weave`を新設し、(1) Weave自身の実行時エラー(`weavert.Add`の型不一致)をガード用クロージャーのパターンで回復するケース、(2) Go資産由来のパニック(`strconv.Atoi`+`raiseIfError`)を同じパターンで回復するケース、(3) `main`自身の本体で直接`recover(...)`を呼ぶケース、の3パターンを実地検証した(`amivm`→`go build`→実行、終了コード`0`)。誤用パターン(オブジェクトリテラルのフィールド値である関数の内部・その入れ子のクロージャー内での`recover(...)`)も個別に実地検証し、いずれも分かりやすいコンパイルエラーで拒否されることを確認した。単体テスト(sema/codegen/weavert)を新規追加、全て green。既存の全19サンプル+`examples/modules`+`examples/multifile`の回帰(`make test-examples`)も無変更で動作継続することを確認した。weave_spec.md §6.5・§11・§13・§15.5・§19.2・§19.5・§19.7・新設§20、`tour/08-wrapup.md`、`weave_implementation_notes.md`(§0の命令数・§19新設)を更新した。

### 組み込み関数の強化: 文字列操作・数学関数・`exit`(「検討中の今後の対応」§1として検討済みだった論点を実装)

`recover(...)`実装完了後に持ち越されていた「組み込み関数の強化」を実施した。19.9節が挙げていた文字列操作の欠落・`exit`相当の手段の欠落に加え、「これらと横並びで必要そうなもの」として数学関数・文字列の検索/変形/分割結合系・乱数/時刻系を洗い出し、`AskUserQuestion`で4カテゴリ(検索・判定/変形/分割・結合/その他)×最大4択のmultiSelectをユーザーへ提示した——CLAUDE.md開発の進め方8「大きな変更はユーザーのOKを得てから着手する」に基づき、実装候補の洗い出しと実装対象の選定を分離した(列挙は私が行い、選定はユーザーが行う、という役割分担をユーザー自身が指示した形)。

**選定結果**: `contains`/`indexOf`(検索・判定)、`substring`/`upper`/`lower`/`trim`/`replace`(変形)、`split`/`join`/`repeat`/`toNumber`(分割・結合・変換)、`exit`/数学関数一式(その他)——**選定されなかったもの**: `startsWith`/`endsWith`、`random()`、`now()`/`sleep(seconds)`。19.9節にはこれら不採用項目を「現状の仕様でできないこと」として明記した(14節「開発の進め方」14の方針通り)。

**実装は既存の「固定引数個数のGo-native呼び出し」パターン(`genOneArgBuiltin`/`genTwoArgBuiltin`)をそのまま踏襲し、新しい設計判断はほとんど不要だった。** 唯一の新規追加は`genThreeArgBuiltin`(`substring`/`replace`が3引数を取るため)——既存の2つの相似形をそのまま3引数へ拡張しただけで、新しい概念は無い。この「recoverが唯一の例外だった」という位置づけ(直前の節)は今回の20個の追加でも崩れていない——全て`?weavert.Xxx`への通常の`CALL`一本化パターンに収まった。

**実装時に決めた細部の判断(いずれも実装の外に持ち出すほどの大きさではないと判断し、ユーザーに個別確認せず進めた)**:

- **`indexOf`の「見つからない」は`-1`を返す(エラーにしない)。** `at(...)`が範囲外アクセスを即座にエラーにする方針(11節)とは対照的——`at(...)`にとって範囲外は「プログラミングミスの可能性が高い」が、`indexOf`にとって「見つからない」は検索の正常な結果の一つ(多くの言語の慣習と同じ)、という性質の違いを踏まえた
- **`substring`/`indexOf`は文字(Unicode)単位でインデックスを扱う。** `len(...)`が文字数(バイト数ではない)を返す既存の契約(Step2時点の確定判断)と一貫させるため——`strings.Index`はバイトオフセットを返すが、そのオフセットまでの部分文字列を`utf8.RuneCountInString`で数え直すことで文字単位へ変換した(`weavert.IndexOf`)
- **`substring`の範囲外・逆順範囲は即座にエラー。** `ObjAt`/`ObjSetAt`(3.1節)が確立した「インデックスの範囲エラーは静かにクランプせずその場で止める」という方針をそのまま踏襲した
- **`replace`は常に全置換。** 「最初のN個だけ置換」のような部分置換のオプションは、他のどの組み込み関数にも前例が無い可変長引数・オプション引数を新規に持ち込むことになるため見送った
- **`join`は要素がすべて文字列であることを要求し、そうでなければエラー。** `weavert.Object`の走査順は`ObjKeys`(Step8の確定判断)と同じ「パディング済みキーでソート」方式を再利用し、`for k, v in list(...)`が見せる順序と一致させた
- **`repeat`の回数`n`は非負の整数のみ許可。** 小数・負数は`weavert.numArg`/`nonNegativeIntArg`という共有ヘルパーでその場でエラーにする——8節の演算子型エラーと同じ「型が食い違えばその場でパニック」という一貫した方針
- **`sqrt`の負数引数はその場でエラー。** GoのNaNをそのまま`any`として伝播させると、Weaveには「NaNを検知する」組み込み手段が無いため、以降の演算全てに気づかれないまま伝播しかねない——8節の演算子型エラーと同じ扱いにする方が、このプロジェクトの一貫した設計方針(実行時型エラーは早期に分かりやすく止める)に合っている
- **`min`/`max`/`pow`は固定2引数。** 可変長引数の`min`/`max`(他言語でよくある形)は、`list(...)`以外に可変長引数を取る組み込み関数の前例が無いため見送り、他の全ての新規組み込み関数と同じ固定アリティに揃えた
- **`exit(...)`はGoの`defer`/`recover`機構を経由しない(`os.Exit`自身の挙動そのまま)。** `recover(...)`(20節)がアクターのメッセージハンドラ内で使用禁止されている理由(6.5節の「部分的な障害分離は無い」という原則の抜け道を塞ぐため、直前の節参照)とは異なり、`exit(...)`は常にプロセス全体を無条件に終了させるだけで、フォールトトレランスの抜け道を作る余地が無い——そのため`exit(...)`の呼び出し可能な場所に制限は設けていない(sema側の変更は`builtinNames`への追加のみ)
...
### amivmの整数リテラル拡張(16進/8進/2進・桁区切り`_`)を受けてWeave自身のlexerにも同じ書き方を追加

ユーザーから「amivmの整数リテラルに16進(`0x`)/8進(`0o`)/2進(`0b`)・桁区切りの`_`が追加された」と伝えられ、amivmリポジトリのコミット(`22c701f`)と`ignored/amivm/amivm_spec.md`(6節)を確認した。**amivm側はレキサー限定の変更(`cmd/amivm/token.go`の`rePosInt`/`reNegInt`という正規表現を拡張しただけ)で、Kind分類(`astbuild.go`/`category.go`)は無変更だった**——Goが1.13以降この構文をネイティブにサポートするため、生成コードには元のトークンをそのまま出力するだけで済んでいる。

**Weave側の`codegen`・生成IRへの影響を先に調査したところ、「実装を修正しなければいけない箇所」は無いと判明した。** Weaveは全ての数値を`float64`として扱い(2節の確定判断)、IRへ出力する際は常に`codegen.formatNumberLiteral`が生成する10進小数形(`26.0`のような、無型浮動小数点定数のデフォルト型をfloat64にするための強制`.0`付与——Step2の確定判断)しか使わない。`internal/codegen/*.go`を`grep`し、Weaveが数値リテラルをIRへ直接埋め込む箇所がこの1つのヘルパーに集約されていること、`ARTYPE`の`imm`のようなコンパイル時整数リテラルを要求するカテゴリを一つも使っていないことを確認し、amivm側の変更がWeaveの生成物を壊す経路が無いことを裏付けた。

**「修正した方が良い箇所」は無いわけではなく、Weave自身のソース言語(`.weave`)が16進/8進/2進リテラル・桁区切り`_`を書けないという既存の制限(weave_spec.md §1.0/§18.1に明記済み)を、この機会に解消するかどうかという判断が残った。** ただしこの制限の起源はamivmのIR側の制約とは無関係だった——Weaveは数値リテラルを常に`strconv.ParseFloat`で`float64`へ変換するだけで、AMIVM-IRには変換後の値をどのみち10進小数形で出力するため、amivm側が16進リテラルをIRとして表現できるかどうかはWeave側の対応可否と論理的には無関係(Weaveの制限は最初から独立して存在していた)。とはいえ「AMIVM-IRの整数リテラル形式が広がった今、Weave自身も対応する方が自然」という判断について、新しい構文の追加(開発の進め方8)にあたるため、ユーザーへ`AskUserQuestion`で意向を確認し、「追加する(推奨)」の選択を得てから実装した。

**実装は`internal/lexer/lexer.go`の`lexNumber`を全面的に書き換え、`0x`/`0X`(16進)・`0o`/`0O`(8進)・`0b`/`0B`(2進)プレフィックスを検出して`lexRadixNumber`へ分岐、それ以外は従来通り`lexDecimalNumber`(10進整数・小数)へ渡す形にした。** 桁区切り`_`の妥当性検査(基数プレフィックス直後、または数字と数字の間にのみ許可——先頭・末尾・連続は構文エラー)は`scanDigitsWithSeparators`という共通ヘルパーに集約し、10進・16進・8進・2進のいずれの整数部でも同じ関数で検証する。**amivmの仕様(6節)にある「桁区切りの`_`は基数プレフィックスの直後、または数字と数字の間にのみ置ける」という規則をそのまま踏襲したが、amivmが10進整数の形自体に課している「`0`単体、または`1`-`9`に`0`-`9`が続く形」という制約(先頭ゼロの多桁10進表記、`007`のような形の禁止)は踏襲しなかった**——Weaveの既存のレキサーは元々この制約を持たず(`007`は`7.0`として今も通る)、これは今回の整数リテラル拡張と無関係な既存の10進表記の緩さであり、無関係な既存挙動をこの機会に厳格化するのはスコープ外と判断した(開発の進め方7の「フラットな視点で設計し直してよい」は、あくまで今回touchする機能に関する裁量であり、無関係な既存機能まで一緒に変える理由にはならない)。

**パーサー側(`internal/parser/parser.go`)は新設の`parseNumberLiteral`ヘルパーで、リテラル文字列から`_`を除去した後、`0x`/`0X`/`0o`/`0O`/`0b`/`0B`で始まっていれば`strconv.ParseInt(clean, 0, 64)`(base 0でamivm自身のGo生成コードと同じくGoの整数リテラル構文の基数プレフィックスをそのまま解釈できる)経由で整数値を得てから`float64`へ変換し、それ以外は従来通り`strconv.ParseFloat`を使う形にした。** 16進/8進/2進リテラルは常に整数値になるため(Weaveには浮動小数点の16進表記のような概念が無い)、小数部との組み合わせは字句解析の時点で発生しえない(`lexRadixNumber`はプレフィックス+桁のみを読み、小数点の続きを読まない)。

実地検証(`0x1A`→`26`、`0o17`→`15`、`0b101`→`5`、`1_000_000`→`1e+06`、`0x_1A`(プレフィックス直後の桁区切り)→`26`、`-0x1A`→`-26`)、およびエラーケース(`0x1A_`・`0x__1`・`0x`(桁無し)・`1__000`・`0b2`・`0o8`)がいずれも分かりやすいコンパイルエラーで拒否されることを確認した。`weave_spec.md` §1.0(数値リテラルの形の説明を全面更新)・§18.1(できないことリストから16進/8進/2進・桁区切りを削除)、`tour/01-basics.md`(数値の節に新しい書き方の例を追加)、`examples/scalars.weave`(既存の数値サンプルに新しい形を追加——新しい決定版サンプルを別ファイルとして起こすのではなく、既存のスカラー値サンプルを拡張する形にした。開発の進め方12「同じパターンの重複は避ける」)を更新した。単体テスト(lexer/parser)を新規追加、全て green。`weave_implementation_notes.md`への影響は無し(このドキュメントはAMIVM-IR生成の知見を記録するものだが、今回の変更はWeave自身のソース言語レキサーに閉じており、IR生成ロジック・生成されるIRの形はどちらも一切変わっていないため、追記すべき新しい知見が無い)。既存の全22サンプル+`examples/modules`+`examples/multifile`の回帰(`make test-examples`)、`gofmt`/`go vet`/`go test`も無変更で動作継続することを確認した。
...
（現時点で未着手の項目は無し。新しい候補が挙がり次第ここに追記する。）
