...
- **文字列操作**: 19.9節「文字列操作の組み込み関数がほとんど無い(`+`による結合と`string()`変換以外、部分文字列取得・分割・置換・トリム等が無い)」が該当する。現状これらはGo資産連携(15節、`gofunc("?strings.XXX")`)経由でしか行えない——組み込み関数として独立させるか、あるいは「これで十分」として現状維持するかも論点になりうる
- **`exit`**: `main`の`return`(12節)以外から、プログラムを即座に終了させる手段が無い現状への対応と思われる。`os.Exit`相当の組み込み関数(`exit(code)`)を想定していると見られるが、詳細(アクターのハンドラ内から呼んだ場合の扱い、`recover(...)`(20節)相当の後始末との関係)は未確認

具体的な候補関数一覧・優先順位は次回の協議待ち。
