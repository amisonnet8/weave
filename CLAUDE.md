...
- **IR化の方針**: AMIVM側にGoの`recover()`に相当する命令があるか(無ければ`weavert`側でGoの`defer`/`recover()`を使ったヘルパーへ一元化することになる見込み——これまでの「動的型付け言語はAMIVMのネイティブ命令と相性が悪く、最初からweavertへ一元化してよい」という教訓(`weave_implementation_notes.md` §1)がそのまま当てはまる可能性が高い)
