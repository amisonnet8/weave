...
ユーザーの想定は「`try`/`catch`ではなく、`defer`のような形で呼んでおくとパニックを捕捉できる」イメージ。**既存の設計方針(weave_spec.md §19.2「例外処理(`try`/`catch`)は無い。エラーは実行時パニックとしてプログラム全体を即座に停止させる形でしか起こらない」・§6.5「メッセージハンドラ内のエラーはプログラム全体を停止させる」)との整合をどう取るかが最大の論点**——本格的な検討はまだ行っていない。

想定される検討事項(未着手、優先順位も未確定):

- **対象範囲**: 15節のGo資産呼び出し(`gofunc`/`gomethod`)由来のGo panicのみを対象にするか、Weave自身の実行時エラー(`ASSERT`失敗・`weavert`の`TypeError`等)も含めるか——後者まで含めると§19.2の「例外処理は無い」という設計原則そのものの見直しに近づくため、範囲の線引きが最初の論点になる
- **構文**: `defer`という名前・キーワードをそのまま再利用するのか(Goの`defer`とは意味が異なる可能性があるため要検討)、別の予約名を新設するのか——`gotype`/`gofunc`/`package(...)`と同じ「`名前 = 予約された関数呼び出し(...)`」という既存パターンを踏襲できるかどうかも論点
- **アクターとの関係**: メッセージハンドラ内で使えるようにするか。6.5節の「部分的な障害分離は無い(1つのアクターだけがクラッシュして他は動き続ける、という設計にはしない)」という既存の意図的な設計判断と、ハンドラ単位でのパニック捕捉が両立するかどうかを最初に整理する必要がある
- **IR化の方針(確定): AMIVMは変更しない。既存の`DEFER callname args...`(`defer callname(args...)`)命令+`weavert`側のヘルパー(内部で普通にGoの`recover()`を呼ぶ)で完結させる。** `DEFER ?weavert.XXX`が生成する`defer weavert.XXX(...)`は素直で正当なGoであり、`recover()`はGoのただの組み込み関数なので摩擦なく呼べる——`CLOS`ネスト許可・`METHOD`命令の2件(どちらも「Goコンパイラに拒否される/原理的に表現不能」という回避策ゼロの状況だったからこそAMIVM側の変更を要求した)とは性質が異なり、こちらは回避策の側に一切の摩擦が無い。専用命令があればIRの読みやすさは上がるという意見も出たが、同じ理由(あれば綺麗だが無くても困らない)は`IF`/`^bool`制約・`SLMAKE`の`deftype`制約・`MPTYPE`/`MGET`の`^any`非対応でも一貫して見送ってきており、ここだけ基準を変える強い理由は無いと判断した。IRの読みやすさが要るなら`weavert`側のヘルパー関数名(例: `weavert.RecoverPanic`)で解決する
...
- **`exit`**: `main`の`return`(12節)以外から、プログラムを即座に終了させる手段が無い現状への対応と思われる。`os.Exit`相当の組み込み関数(`exit(code)`)を想定していると見られるが、詳細(アクターのハンドラ内から呼んだ場合の扱い、`defer`相当の後始末との関係——上記1の検討と絡む可能性がある)は未確認
