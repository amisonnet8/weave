...
### 1. 同名ディレクトリを持つ2パッケージ取り込み時の束縛名衝突(§17.4/§19.8で「既知の未検出の不具合」として記録済み)

ユーザーから「`package(...)`の代入先の変数名を変えれば回避できるのでは?回避できないなら回避できるようにしたい」と問われ、実装(`internal/modloader/modloader.go`の`collectRenames`)を確認・実地検証したところ、**代入先の変数名を変えても回避できないことを確認した。**

**理由**: リネーム用プレフィックスは取り込み側の変数名(`quals`テーブルのキー)ではなく、**取り込まれる側のディレクトリ名(`loc.name`、`filepath.Base`)だけ**で決まる(`collectRenames(file.TopLevel, loc.name)`)。別々の絶対パスにある2つのパッケージが同じベース名(例: `foo/utils`と`bar/utils`)を持つ場合、どちらも同じ`utils_X`という名前へリネームされ、後から読み込まれた方が先の方を静かに上書きする。取り込み側が`package(...)`の代入先変数名(`utilsA`/`utilsB`等)を変えても、この内部プレフィックスには一切影響しない。

**実地検証(再現)**:

```
utilsA = package("./a/utils")   // Helper = fn(x) { return "A:" + string(x) }
utilsB = package("./b/utils")   // Helper = fn(x) { return "B:" + string(x) }

main = fn(args) {
	print(utilsA.Helper(1))   // 期待: "A:1"
	print(utilsB.Helper(2))   // 期待: "B:2"
	return 0
}
```

実行すると`B:1`/`B:2`が出力される(`utilsA.Helper`が`utilsB.Helper`の定義で上書きされている)。コンパイルエラーにはならず、終了コードも正常なため、この検証で明示的に比較するまで気づきにくい。

**修正案(第1版・未実装)**: `loader`が今回の`Load()`呼び出し全体で使ったベース名を追跡する仕組みを新設し、同じベース名を持つ**別の絶対パス**を2つ目以降取り込んだ場合だけ、自動的に連番サフィックス(`utils2`,`utils3`,...)を付けて内部の一意化プレフィックスを衝突しない形にする案を最初に提示した。

**ユーザーからのフィードバック: 「言語を使う側としては別名変数で回避が一番スマート」——自動採番より、ユーザー自身が制御できる形の方が良いという指摘。** これを受けて再設計した。

**修正案(第2版・未実装、こちらを採用する方向)**: 内部の一意化プレフィックスを、**取り込まれる側のディレクトリ名(`loc.name`)ではなく、取り込む側が`package(...)`の代入先に選んだ変数名**から決めるように変更する。

- `extractPackageDecls`内の`ld.loadPackage(targetLoc, "", false)`呼び出しへ、その場で分かっている代入先の変数名(`a.Name`)を渡し、`loadPackage`は(そのパッケージを初めてロードする際に限り)`collectRenames(file.TopLevel, loc.name)`の`loc.name`の代わりにこの変数名を使ってリネームする。冒頭の再現例(`utilsA = package("./a/utils")` / `utilsB = package("./b/utils")`)は、この変更だけで**ユーザーの直感通り**衝突しなくなる見込み(`utilsA_Helper` / `utilsB_Helper`という別々の名前になるため)
- 同じ絶対パスのパッケージが既にロード済み(`ld.loaded`にキャッシュ済み)の場合、2回目以降の`package(...)`呼び出しがどんな変数名を使おうと、内部の一意化プレフィックスは**最初にロードされた時に使われた変数名のまま**——1つの絶対パスにつき評価・リネームは1回だけ、という既存の原則(§17.4)を崩さない
- エラーメッセージに出す表示用の名前(`loadedPackage.Name`、循環import・非公開メンバー参照等のエラー文言に使われる)はディレクトリ名のまま変更しない——変わるのはコンパイラ内部の一意化プレフィックスだけで、`internal/modloader/rewrite.go`の`tryResolveQualified`は`target.Renames[e.Prop]`(既に完成した最終フラット名)を引くだけなので、プレフィックスの決め方が変わっても無改修で動くはず(実装前の分析時点での見立て)
- **新たに生まれる論点: 今度は「同じ変数名を、別々の場所で別々のパッケージへ割り当てた」場合に、形を変えた同じ問題が起こりうる**(例: あるファイルが`utils = package("./foo/utils")`、別のファイルが`utils = package("./bar/utils")`——変数名の重複は、ディレクトリ名と違って各パッケージ内で自由に選べる分、意図せず起こりうる)。**ユーザーと協議の上、これは分かりやすいコンパイルエラーにする方針で確定した**(「変数名`utils`は既に別のパッケージ(`<path1>`)の取り込みに使われています」のような、ユーザー自身にどちらかの変数名を変えてもらうエラー)——自動で連番を振って黙って回避する案は不採用。このプロジェクトが一貫して採用してきた「ユーザーが直せる曖昧さは黙って進めずエラーにする」という方針(15.4節の型ヒント不一致・`gomethod`/`gofunc`のall-or-nothing違反等)にも、「ユーザー自身が変数名で制御する」という今回の指摘の精神にも合致する
...
- **`exit`**: `main`の`return`(12節)以外から、プログラムを即座に終了させる手段が無い現状への対応と思われる。`os.Exit`相当の組み込み関数(`exit(code)`)を想定していると見られるが、詳細(アクターのハンドラ内から呼んだ場合の扱い、`defer`相当の後始末との関係——上記2の検討と絡む可能性がある)は未確認
